...
//
// Explicit CLI actions sit in front of the hook path: `set-license` (store the customer
// key), `login` (get one through the browser), `exec` (drive a headless agent through the
// review loop), `review-pr` (review a pull request's diff), `mcp` (serve the read tools over
// the Model Context Protocol on stdio — LEO-88, built but withheld from every release) and
// `stats` (print those same figures as JSON for a slash command to summarise — LEO-248).
//
...
func main() {
	...
	// `review-pr` reviews a PULL REQUEST's diff and posts the findings as review
	// comments (LEO-231). Same reasoning as `exec`: a CI job fires no Stop hook, the
	// output is human-facing step-log text rather than the re-wake JSON, and it must
	// never reach run() — which would read the runner's empty stdin as an unparseable
	// hook event and fail open silently.
	if len(os.Args) > 1 && os.Args[1] == "review-pr" {
		os.Exit(runReviewPR(os.Args[2:]))
	}
	...
	// `stats` prints the developer's own figures as the dashboard's JSON (LEO-248) — the
	// SHIPPED route to the read API that `mcp` above serves but no release declares. A slash
	// command runs it through Bash, so it needs none of the MCP consent dialog that keeps
	// those tools withdrawn. Human/agent-facing stdout, not the re-wake channel, so like
	// every other explicit action it must never reach run().
...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if ev.IsUserPromptSubmit() {
		if cfg, err := config.Load(); err == nil && cfg.Tier == config.TierCloud {
			engine.CaptureRecovery(a, ev)
		}
