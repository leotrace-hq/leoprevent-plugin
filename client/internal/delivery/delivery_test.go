...
func TestCloudKeepsConnectorFindingsOutOfFixOutcome(t *testing.T) {
	srv := stubServer(t, "", `{"verdict":"triggered","review_id":"test-review","summary":"Fix the finding","findings":[{"rule":"eval-injection","name":"Code / Eval Injection","location":"mcp/mcp__webflow_test__register_inline_script/call-1/source_code.js:1","issue":"unsafe eval","fix":"remove eval"}]}`)
	h := Cloud{client: apiclient.New(srv.URL, "")}
	changes := []transcript.Change{{FilePath: "mcp/mcp__webflow_test__register_inline_script/call-1/source_code.js", AddedText: "eval(location.hash)", Virtual: true}}
	got, err := h.Review(t.TempDir(), changes, wire.TurnMeta{})
	if err != nil {
		t.Fatal(err)
	}
	if got.Prompt == "" || got.Pending != nil || len(got.Findings) != 1 || !got.Findings[0].Virtual {
		t.Fatalf("connector finding was not report-only: %+v", got)
	}
	if got.Summary != "" || strings.Contains(got.Prompt, "fix before finishing") || !strings.Contains(got.Prompt, "not files to edit") {
		t.Fatalf("connector prompt asked for an impossible fix: %+v", got)
	}
}

func TestCloudMixedReviewTracksOnlyEditableFinding(t *testing.T) {
	srv := stubServer(t, "", `{"verdict":"triggered","review_id":"mixed-review","findings":[{"rule":"eval-injection","location":"mcp/call/source_code.js:1","issue":"unsafe eval","fix":"remove eval"},{"rule":"sql-injection","location":"app.py:1","issue":"unsafe query","fix":"parameterize"}]}`)
	h := Cloud{client: apiclient.New(srv.URL, "")}
	changes := []transcript.Change{{FilePath: "app.py", AddedText: "query(user)"}, {FilePath: "mcp/call/source_code.js", AddedText: "eval(location.hash)", Virtual: true}}
	got, err := h.Review(t.TempDir(), changes, wire.TurnMeta{})
	if err != nil {
		t.Fatal(err)
	}
	if got.Pending == nil || len(got.Pending.Findings) != 1 || got.Pending.Findings[0].Location != "app.py:1" || len(got.Pending.Before) != 1 || got.Pending.Before[0].Path != "app.py" {
		t.Fatalf("mixed review tracked the connector call as a fix: %+v", got.Pending)
	}
	if review.ForceFixedCount(got.Findings) != 1 {
		t.Fatalf("mixed review fix count = %d, want 1", review.ForceFixedCount(got.Findings))
	}
}
...
func TestServerNoticeFlowsThroughBothTiers(
	t *testing.T,
) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusForbidden)
		_, _ = w.Write([]byte(`{"error":"denied","notice":{"id":"new.reason","message":"Server selected this wording","action_url":"https://example.com/help"},"retry_with_new_key":false}`))
	}))
	defer server.Close()
	for _, tier := range []string{config.TierLocal, config.TierCloud} {
		reviewer, err := New(&config.Config{ServerURL: server.URL, Tier: tier})
		if err != nil {
			t.Fatal(err)
		}
		_, err = reviewer.Review("", ssrfChange, wire.TurnMeta{})
		var skipped *review.SkipError
		if !errors.As(err, &skipped) || skipped.Notice == nil || skipped.Notice.ID != "new.reason" || !skipped.KeepKey {
			t.Fatalf("%s lost server notice: %v", tier, err)
		}
	}
}

func TestCloudSendsTypeScriptAliasContext(
	t *testing.T,
) {
	root := t.TempDir()
	...
	for rel, body := range map[string]string{
		...
		"src/auth.ts":   "export function verifyAdminToken() { return token.role === 'admin'; }",
	} {
	...
	t.Cleanup(srv.Close)
	changes := []transcript.Change{{FilePath: "src/app.ts", FullContent: "import { verifyAdminToken } from '@/auth';", AddedText: "if (!verifyAdminToken()) return 403;"}}
	for _, enabled := range []bool{true, false} {
		got = wire.ReviewRequest{}
		h := Cloud{client: apiclient.New(srv.URL, ""), resolveImports: enabled}
		if _, err := h.Review(root, changes, wire.TurnMeta{}); err != nil {
			t.Fatal(err)
		}
		if enabled {
			if len(got.Context) != 1 || got.Context[0].Path != "src/auth.ts" || !strings.Contains(got.Context[0].Content, "token.role === 'admin'") {
				t.Fatalf("missing alias guard body: %+v", got.Context)
			}
		} else if len(got.Context) != 0 {
			t.Fatalf("context sent with resolution disabled: %+v", got.Context)
		}
	}
}

// The wire-level counterpart to the resolver's own unit test: on a NodeNext repo the
// specifier the source carries is `./authorize.js` while the file is `authorize.ts`, and
// before LEO-224 that resolved to nothing, so the ReviewRequest reaching the server had an
// EMPTY Context. Both the selector and the judge read the same rendered payload, so an
// empty Context is a blind spot for both passes and every rule, not only the one that
// motivated the ticket. This asserts the guard body is actually on the wire.
func TestCloudSendsNodeNextEmittedSpecifierContext(
	t *testing.T,
) {
	root := t.TempDir()
	...
	for rel, body := range map[string]string{
		...
		"src/middleware/authorize.ts": "export const requireAuthor = (c, next) => " +
			"{ if (c.role !== 'admin' && c.role !== 'author') return c.json({}, 403); return next() }",
	} {
		name := filepath.Join(root, rel)
		if err := os.MkdirAll(filepath.Dir(name), 0o755); err != nil {
			t.Fatal(err)
	...
	changes := []transcript.Change{{
		FilePath:    "src/routes/lab-admin.ts",
		FullContent: "import { requireAuthor } from '../middleware/authorize.js';",
		AddedText:   "labAdminRoute.post('/rescore', requireAuthor, handler)",
	}}
	h := Cloud{client: apiclient.New(srv.URL, ""), resolveImports: true}
	if _, err := h.Review(root, changes, wire.TurnMeta{}); err != nil {
		t.Fatal(err)
	}
	if len(got.Context) != 1 || got.Context[0].Path != "src/middleware/authorize.ts" ||
		!strings.Contains(got.Context[0].Content, "c.role !== 'author'") {
		t.Fatalf("guard body absent from the request the server received: %+v", got.Context)
	}
}

func TestSpendLimitKeepsCredentialsAndNotice(
	t *testing.T,
) {
	keep := false
	notice := &wire.ServerNotice{ID: "spend_limit", Message: "This turn was NOT reviewed."}
	err := classifyServerErr(&apiclient.StatusError{Path: "/review", Status: http.StatusPaymentRequired, Notice: notice, RetryWithNewKey: &keep})
	var skipped *review.SkipError
	if !errors.As(err, &skipped) || skipped.Notice != notice || !skipped.KeepKey || skipped.Reason != review.SkipServerError {
		t.Fatalf("spend limit classification=%+v", skipped)
	}
}

// The server's summary line describes one request, so it reaches the notice only when the
// turn fitted in one /review call; a split turn composes its own.
func TestCloudSummaryPassesThroughOnlyForASingleBatch(t *testing.T) {
	serve := func() *httptest.Server {
		return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			_, _ = w.Write([]byte(`{"verdict":"triggered","review_id":"rv","summary":"Server says hello.","findings":[{"rule":"ssrf","location":"a.py:1"}]}`))
		}))
	}
	one := serve()
	t.Cleanup(one.Close)
	r, _ := New(&config.Config{ServerURL: one.URL, Tier: config.TierCloud})
	res, err := r.Review("", []transcript.Change{{FilePath: "a.py", AddedText: "x = 1\n"}}, wire.TurnMeta{})
	if err != nil || res.Summary != "Server says hello." {
		t.Fatalf("single batch: summary %q, err %v", res.Summary, err)
	}

	two := serve()
	t.Cleanup(two.Close)
	big := strings.Repeat("x", limits.MaxReviewBytes/2+1024)
	r, _ = New(&config.Config{ServerURL: two.URL, Tier: config.TierCloud})
	res, err = r.Review("", []transcript.Change{{FilePath: "a.py", AddedText: big}, {FilePath: "b.py", AddedText: big}}, wire.TurnMeta{})
	if err != nil || res.Summary != "" {
		t.Fatalf("split turn must drop the per-batch summary: %q, err %v", res.Summary, err)
	}
}
