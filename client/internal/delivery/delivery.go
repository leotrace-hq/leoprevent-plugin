...
import (
	...
	"github.com/leotrace-hq/leoprevent-plugin/imports"
...
func (h Cloud) Review(cwd string, changes []transcript.Change, meta wire.TurnMeta) (engine.Result, error) {
	...
	if h.resolveImports {
		var localChanges []transcript.Change
		for _, change := range changes {
			if !change.Virtual {
				localChanges = append(localChanges, change)
			}
		}
		ctx = resolveContext(cwd, localChanges)
		if len(ctx) > 0 {
	...
	for bi, batch := range batches {
		...
		resp.Summary = br.Summary
		// Every batch of one turn is one repository on one account, so the first hint is the hint.
		if resp.ContextHint == "" {
			resp.ContextHint = br.ContextHint
		}
	}
	// The server's summary describes ONE request, so a turn split across batches composes
	// its own rather than showing one batch's account as the whole turn's.
	if len(batches) != 1 {
		resp.Summary = ""
	}
	if len(batches) > 1 {
	...
	for _, change := range changes {
		if change.Virtual {
			virtualPaths[change.FilePath] = true
		}
	}
	var tracked []wire.Finding
	for i := range resp.Findings {
		for path := range virtualPaths {
			if resp.Findings[i].Location == path || strings.HasPrefix(resp.Findings[i].Location, path+":") {
				resp.Findings[i].Virtual = true
				break
			}
		}
		if !resp.Findings[i].Virtual {
			tracked = append(tracked, resp.Findings[i])
		}
	}
	...
	res := engine.Result{Prompt: prompt, Findings: resp.Findings, Summary: resp.Summary, ContextHint: resp.ContextHint}
	if prompt != "" && len(tracked) > 0 {
		var trackedFiles []wire.ChangedFile
		for _, file := range files {
			if !virtualPaths[file.Path] {
...
func (h Cloud) ShipOutcome(p outcome.Pending, after []transcript.Change, agentResponse string, meta wire.TurnMeta) ([]wire.Finding, []wire.Finding, error) {
	afterFiles := make([]wire.ChangedFile, 0, len(after))
	for _, c := range after {
		if c.Virtual {
			continue
		}
		afterFiles = append(afterFiles, wire.ChangedFile{Path: c.FilePath, AddedText: c.AddedText, FullContent: c.FullContent, AddedLines: c.AddedLines})
	}
...
func classifyServerErr(err error) error {
	...
	if errors.As(err, &se) {
		reason := review.SkipServerError
		if se.Status == http.StatusUnauthorized || se.Status == http.StatusForbidden {
			reason = review.SkipUnauthorized
		}
		return &review.SkipError{
			Reason: reason, Err: err, Notice: se.Notice,
			KeepKey: se.RetryWithNewKey != nil && !*se.RetryWithNewKey,
		}
	}
...
func resolveContext(cwd string, changes []transcript.Change) []wire.ContextFile {
	...
	for _, rel := range order {
		...
		// The resolver takes its own Change (it lives outside client/internal now, so it
		// cannot name transcript's); the fields are identical, so this is a straight copy.
		ics := make([]imports.Change, 0, len(byRepo[rel]))
		for _, c := range byRepo[rel] {
			ics = append(ics, imports.Change{
				FilePath:    c.FilePath,
				AddedText:   c.AddedText,
				FullContent: c.FullContent,
				AddedLines:  c.AddedLines,
			})
		}
		for _, cf := range imports.Resolve(root, ics) {
