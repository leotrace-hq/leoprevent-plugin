...
func BuildPrompt(changes []transcript.Change, selected []rulespec.Rule, metaPolicy string) string {
	...
	if virtual {
		b.WriteString("- For MCP connector call inputs, report the issue and fix to the developer. The call was already sent and its synthetic path is not a file. Do not claim the call was changed or undone.\n")
		b.WriteString("- Apply every other fix without asking, EXCEPT rules marked SUGGEST-ONLY. For those, report the issue + fix and let the developer decide; do not change the code.\n")
	} else {
...
func BuildFindingsPrompt(findings []wire.Finding, preexistingText string) string {
	...
	var introduced, suggestOnly, preexisting, virtual []wire.Finding
	for _, f := range findings {
		switch {
		case f.Virtual:
			virtual = append(virtual, f)
		case f.SuggestOnly:
	...
	case len(suggestOnly) > 0 || len(preexisting) > 0 || len(virtual) > 0:
	...
	if len(virtual) > 0 {
		b.WriteString("These findings are in code already sent through an MCP connector call. The cited paths are review labels, not files to edit. Tell the developer what was sent, the risk, and how to correct a future call. Do not claim the completed call was changed or undone:\n\n")
		writeFindingGroups(&b, virtual)
	}
...
func ReviewContextMessage(fileCount int, findings []wire.Finding, forceFixed int) string {
	return ReviewContextMessageWith("", fileCount, findings, forceFixed)
}

// maxSummaryRunes bounds the server's summary line. It is one sentence; anything longer is
// not the field it was meant to be.
const maxSummaryRunes = 300

// cleanSummary makes the server's summary safe to print as one line of the notice: no line
// breaks (a second line would escape the quoted block), trimmed, and bounded. Empty ⇒ use the
...
// AppendContextHint puts the server's repository-context reminder (wire.ReviewResponse.ContextHint)
// at the very END of a re-wake prompt, after every finding: the re-wake's job is getting the flaw
// fixed, and an unrelated request placed earlier competes with that instruction (LEO-113's lesson).
// The sentence is the server's, rendered verbatim on one line under its own bound;
// an empty hint, or an empty prompt, is returned unchanged.
func AppendContextHint(prompt, hint string) string {
	// Its OWN bound, never the summary's: the summary is a short line and this sentence is not,
	// and sharing the 300-rune cut once delivered the reminder chopped off mid-word.
	line := strings.Join(strings.Fields(hint), " ")
	if r := []rune(line); len(r) > limits.MaxContextHintRunes {
		line = string(r[:limits.MaxContextHintRunes])
	}
	if line == "" || prompt == "" {
		return prompt
	}
...
// ReviewContextMessageWith is ReviewContextMessage with the server's summary line
// (wire.ReviewResponse.Summary) in place of the client's own when one was sent. The byline and
// the instruction around it are unchanged either way.
func ReviewContextMessageWith(summary string, fileCount int, findings []wire.Finding, forceFixed int) string {
	if line := cleanSummary(summary); line != "" {
		return contextWrap("> " + line)
	}
	...
	//
	// ⚠️ AN AIKIDO DISPUTE IS NAMED APART FROM THE FINDINGS, BECAUSE IT IS NOT ONE. It is an
	// existing Aikido finding the review thinks is a false positive, put to the agent to confirm,
	// and counted nowhere. Folded into "raised N findings" it told the developer the review had
	// found a flaw in a turn where it had found none. The server marks one by its rule prefix
	// (wire.AikidoDisputeRulePrefix), which is a fact about the finding rather than a policy.
	disputes := AikidoDisputeCount(findings)
	snykDisputes := SnykDisputeCount(findings)
	raised := findings[:0:0]
	for _, f := range findings {
		if !strings.HasPrefix(f.Rule, wire.AikidoDisputeRulePrefix) && !strings.HasPrefix(f.Rule, wire.SnykDisputeRulePrefix) {
			raised = append(raised, f)
		}
	}
	aikido := ""
	if disputes > 0 {
		aikido = count(disputes, "Aikido finding") + " to double-check"
	}
	if snykDisputes > 0 {
		if aikido != "" {
			aikido += ", plus "
		}
		aikido += count(snykDisputes, "Snyk finding") + " to double-check"
	}
	var tail string
	switch {
	case len(raised) == 0 && disputes+snykDisputes > 0:
		tail = " and found " + aikido + ". See below."
	case len(findings) == 0:
	...
	case forceFixed == 0:
		tail = " and raised " + count(len(raised), "finding") + withAikido(aikido) + ". See below."
	case forceFixed == len(raised) && disputes+snykDisputes == 0:
		tail = " and flagged " + count(forceFixed, "finding") + " to fix below."
	case forceFixed == len(raised):
		tail = " and flagged " + count(forceFixed, "finding") + " to fix" + withAikido(aikido) + ". See below."
	default:
		tail = ", flagged " + count(forceFixed, "finding") + " to fix and raised " +
			strconv.Itoa(len(raised)-forceFixed) + " more" + withAikido(aikido) + ". See below."
	}

	return contextWrap(head + tail)
}

// contextWrap puts one quoted summary line inside the notice's byline and instruction.
func contextWrap(line string) string {
	return "Begin your reply with exactly this markdown, before anything else:\n\n" +
		...
		"> \n" +
		line + "\n\n---\n\n" +
		"Then continue with the developer's request as normal, reporting the review " +
		"result and any changes you made."
}

// withAikido appends the dispute clause to a sentence that already counted findings.
func withAikido(clause string) string {
	if clause == "" {
		return ""
	}
	return ", plus " + clause
...
// AikidoDisputeCount is how many findings are Aikido findings put to the agent to confirm as
// likely false positives. See ReviewContextMessage.
func AikidoDisputeCount(findings []wire.Finding) int {
	n := 0
	for _, f := range findings {
		if strings.HasPrefix(f.Rule, wire.AikidoDisputeRulePrefix) {
			n++
		}
	}
	return n
...
func ForceFixedCount(findings []wire.Finding) int {
	n := 0
	for _, f := range findings {
		if !f.SuggestOnly && !f.Preexisting && !f.Virtual {
			n++
		}
	}
	return n
...
func SnykDisputeCount(findings []wire.Finding) int {
	n := 0
	for _, f := range findings {
		if strings.HasPrefix(f.Rule, wire.SnykDisputeRulePrefix) {
			n++
		}
	}
	return n
