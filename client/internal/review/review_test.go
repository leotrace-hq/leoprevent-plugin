...
func TestVirtualConnectorFindingReportsWithoutClaimingAFileFix(t *testing.T) {
	f := wire.Finding{Rule: "eval-injection", Location: "mcp/call/source_code.js:1", Issue: "unsafe eval", Fix: "remove eval", Virtual: true}
	prompt := BuildFindingsPrompt([]wire.Finding{f}, "")
	if ForceFixedCount([]wire.Finding{f}) != 0 || strings.Contains(prompt, "fix before finishing") || !strings.Contains(prompt, "not files to edit") || !strings.Contains(prompt, "Do not claim the completed call was changed or undone") {
		t.Fatalf("virtual finding prompted an impossible edit: %s", prompt)
	}
	context := ReviewContextMessage(1, []wire.Finding{f}, 0)
	if strings.Contains(context, "what you changed") || !strings.Contains(context, "any changes you made") {
		t.Fatalf("context implies a fix happened: %s", context)
	}
	local := BuildPrompt([]transcript.Change{{FilePath: "mcp/call/source_code.js", AddedText: "eval(location.hash)", Virtual: true}}, nil, "")
	if !strings.Contains(local, "synthetic path is not a file") {
		t.Fatalf("local tier omitted the connector disposition: %s", local)
	}
}
...
// TestReviewContextMessageNamesAikidoDisputesApart pins that an Aikido finding put to the agent
// as a likely false positive is never counted as a finding the review raised (LEO-332). Folded
// in, a turn where the review found nothing read "raised 1 finding".
func TestReviewContextMessageNamesAikidoDisputesApart(t *testing.T) {
	dispute := wire.Finding{Rule: wire.AikidoDisputeRulePrefix + "nosql", SuggestOnly: true, Preexisting: true}
	introduced := wire.Finding{Rule: "ssrf"}
	surfaced := wire.Finding{Rule: "xss-backend", Preexisting: true}
	cases := []struct {
		name string
		f    []wire.Finding
		want string
	}{
		{"dispute only", []wire.Finding{dispute}, "Reviewed 1 changed file and found 1 Aikido finding to double-check. See below."},
		{"surfaced plus dispute", []wire.Finding{surfaced, dispute}, "Reviewed 1 changed file and raised 1 finding, plus 1 Aikido finding to double-check. See below."},
		{"fix plus dispute", []wire.Finding{introduced, dispute, dispute}, "Reviewed 1 changed file and flagged 1 finding to fix, plus 2 Aikido findings to double-check. See below."},
		{"mixed", []wire.Finding{introduced, surfaced, dispute}, "Reviewed 1 changed file, flagged 1 finding to fix and raised 1 more, plus 1 Aikido finding to double-check. See below."},
		{"no dispute is unchanged", []wire.Finding{introduced}, "Reviewed 1 changed file and flagged 1 finding to fix below."},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			msg := ReviewContextMessage(1, c.f, ForceFixedCount(c.f))
			if !strings.Contains(msg, c.want) {
				t.Fatalf("want %q in\n%s", c.want, msg)
			}
		})
...
func TestReviewContextMessageWithUsesTheServersLine(t *testing.T) {
	fs := []wire.Finding{{Rule: "ssrf"}}
	got := ReviewContextMessageWith("Reviewed 1 changed file and did something new.\nsecond line", 1, fs, 1)
	if !strings.Contains(got, "> Reviewed 1 changed file and did something new. second line\n") {
		t.Fatalf("server line not rendered as one quoted line:\n%s", got)
	}
	if strings.Contains(got, "flagged 1 finding to fix") {
		t.Fatalf("client wording must not appear beside the server's:\n%s", got)
	}
	if !strings.Contains(got, "🔒 **LeoPrevent**") || !strings.Contains(got, "Then continue with the developer's request") {
		t.Fatalf("byline and instruction must stay:\n%s", got)
	}
	if ReviewContextMessageWith("   ", 1, fs, 1) != ReviewContextMessage(1, fs, 1) {
		t.Fatal("a blank server line must fall back to the client's wording")
	}
	long := strings.Repeat("a", 1000)
	out := ReviewContextMessageWith(long, 1, fs, 1)
	line := out[strings.LastIndex(out, "\n> ")+3 : strings.Index(out, "\n\n---")]
	if n := len([]rune(line)); n != maxSummaryRunes {
		t.Fatalf("summary not bounded: %d runes", n)
	}
}

func TestReviewContextMessageNamesSnykAndAikidoDisputes(t *testing.T) {
	fs := []wire.Finding{{Rule: wire.SnykDisputeRulePrefix + "sql", SuggestOnly: true}, {Rule: wire.AikidoDisputeRulePrefix + "sql", SuggestOnly: true}}
	got := ReviewContextMessage(1, fs, ForceFixedCount(fs))
	if !strings.Contains(got, "found 1 Aikido finding to double-check, plus 1 Snyk finding to double-check") || strings.Contains(got, "raised") {
		t.Fatalf("summary %q", got)
	}
}

func TestAppendContextHintIsOneBoundedLineAtTheEnd(t *testing.T) {
	got := AppendContextHint("FIX THIS\n\n", "  Connect GitHub\nplease  ")
	if got != "FIX THIS\n\nConnect GitHub please\n" {
		t.Errorf("got %q", got)
	}
	if AppendContextHint("", "x") != "" {
		t.Error("an empty prompt must stay empty: the hint never makes a block")
	}
	if AppendContextHint("P", "   ") != "P" {
		t.Error("an empty hint changes nothing")
	}
	// A sentence longer than the SUMMARY's bound must arrive whole: sharing that bound once
	// cut the real reminder off mid-word.
	long := strings.Repeat("word ", 80) + "end."
	if got := AppendContextHint("P", long); !strings.HasSuffix(got, "end.\n") {
		t.Errorf("a 400-character hint was cut: %q", got[len(got)-40:])
	}
}
