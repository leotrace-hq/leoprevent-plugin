...
// dashboard's agent API (GET /api/agent).
//
// TWO CONSUMERS, and only one of them ships: the `mcp` subcommand serves these as MCP tools
// (LEO-88, withheld from every release — a plugin declaring a local MCP server makes Claude
// Code show an install consent dialog), and the `stats` subcommand prints them for the
// `/leoprevent:stats` slash command to summarise (LEO-248), which needs no such declaration.
// So this package is what a developer's figures actually reach them through; treat a change
// here as customer-facing rather than as maintenance on a withdrawn feature.
//
...
// needed — which is the point of putting these reads in this binary at all.
//
...
// would simply vanish from the answer with nothing failing. Both consumers hand the body
// straight to the agent, which reads JSON perfectly well.
package stats
