...
func resolve(path string) (*Config, error) {
	...
	u := readUserLicenseFile()
	if u.LicenseKey != "" && (u.ServerURL == "" || (strings.TrimRight(c.ServerURL, "/") == u.ServerURL && strings.TrimRight(c.DashboardURL, "/") == u.DashboardURL)) {
		c.LicenseKey = u.LicenseKey
	}
...
func SaveLoginLicense(
	...
	return saveUserLicense(func(u *userLicense) {
		u.LicenseKey = key
		u.ServerURL = strings.TrimRight(serverURL, "/")
		u.DashboardURL = strings.TrimRight(dashboardURL, "/")
		u.DeviceID = deviceID
	})
}
