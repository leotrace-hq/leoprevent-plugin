...
func TestCustomMCPToolsExtendDefaults(t *testing.T) {
	clearEnv(t)
	c, err := resolve(writeJSON(t, `{"server_url":"https://x","mcp_tools":[{"tool":"mcp__custom__publish_*","fields":{"payload.code":".js"}}]}`))
	if err != nil {
		t.Fatal(err)
	}
	rules := c.MCPToolRules()
	if len(rules) <= 1 || rules[len(rules)-1].Tool != "mcp__custom__publish_*" {
		t.Fatalf("rules = %+v", rules)
	}
}

func TestUserMCPToolsSurvivePluginConfigReplacement(t *testing.T) {
	clearEnv(t)
	userPath, err := UserMCPToolsPath()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(userPath, []byte(`{"mcp_tools":[{"tool":"mcp__custom__publish","fields":{"code":".js"}}]}`), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	rules := c.MCPToolRules()
	if rules[len(rules)-1].Tool != "mcp__custom__publish" {
		t.Fatalf("user mapping absent: %+v", rules)
	}
}

func TestRejectMalformedMCPToolRule(t *testing.T) {
	clearEnv(t)
	for _, body := range []string{
		`{"server_url":"https://x","mcp_tools":[{"tool":"publish_*","fields":{"code":".js"}}]}`,
		`{"server_url":"https://x","mcp_tools":[{"tool":"mcp__[","fields":{"code":".js"}}]}`,
		`{"server_url":"https://x","mcp_tools":[{"tool":"mcp__x__publish","fields":{"code":"../secret"}}]}`,
	} {
		if _, err := resolve(writeJSON(t, body)); err == nil {
			t.Fatalf("accepted malformed rule: %s", body)
		}
	}
}
