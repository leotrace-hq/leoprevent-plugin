...
func TestSpendLimitPreservesNoticeWithoutRetry(
	t *testing.T,
) {
	calls := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"error":"spend_limit","notice":{"id":"spend_limit","message":"This turn was NOT reviewed."},"retry_with_new_key":false}`))
	}))
	defer server.Close()
	_, err := New(server.URL, "existing-key").Review(nil, nil, wire.TurnMeta{})
	var status *StatusError
	if !errors.As(err, &status) || status.Status != http.StatusPaymentRequired || status.Notice == nil || status.Notice.ID != "spend_limit" || status.Notice.Message != "This turn was NOT reviewed." || status.RetryWithNewKey == nil || *status.RetryWithNewKey {
		t.Fatalf("response=%+v err=%v", status, err)
	}
	if calls != 1 {
		t.Fatalf("provider requests=%d", calls)
	}
}
