...
// TestHowLeadsWithTheActionThatWorks pins WHICH action each claude surface is told to
// take first, because both surfaces previously led with a manual click-path or a pair of
// hand-typed slash commands.
//
// Desktop must lead with the RESTART. There is no agent-runnable update there (the app's
// marketplace lives in managed state the standalone CLI cannot see, so `claude plugin
// update leoprevent@leotrace --scope managed` answers "Marketplace leotrace not found"),
// and a desktop install auto-updates — so the build is usually already on disk and the
// click-path is the fallback, not the instruction. Terminal must lead with ASKING THE
// AGENT, since there the agent can actually run it.
//
// Both halves assert ORDER, not just presence: the old copy already contained the
// fallbacks, so a contains-only check would pass against it and pin nothing.
func TestHowLeadsWithTheActionThatWorks(t *testing.T) {
	desktop := How("claude", wire.EnvClaudeDesktop)
	restart := strings.Index(desktop, "restart Claude")
	clickPath := strings.Index(desktop, "Customize → Plugins → Browse → Code")
	if restart < 0 {
		t.Errorf("desktop copy must tell the developer to restart; got %q", desktop)
	}
	if clickPath < 0 {
		t.Errorf("desktop copy must keep the click-path as a fallback; got %q", desktop)
	}
	if restart >= 0 && clickPath >= 0 && restart > clickPath {
		t.Errorf("desktop copy must LEAD with the restart, not the click-path; got %q", desktop)
	}

	terminal := How("claude", wire.EnvClaudeTerminal)
	ask := strings.Index(terminal, "ask Claude to update the LeoPrevent plugin")
	manual := strings.Index(terminal, "/plugin marketplace update leotrace")
	if ask < 0 {
		t.Errorf("terminal copy must offer the prompt-driven update; got %q", terminal)
	}
	if manual < 0 {
		t.Errorf("terminal copy must keep the slash commands as a fallback; got %q", terminal)
	}
	if ask >= 0 && manual >= 0 && ask > manual {
		t.Errorf("terminal copy must LEAD with asking the agent, not the slash commands; got %q", terminal)
	}

	// "ask Claude", never "ask me": How() also feeds Message(), the terminal
	// systemMessage, which is the plugin speaking rather than the agent.
	if strings.Contains(terminal, "ask me") {
		t.Errorf("copy must say \"ask Claude\", not \"ask me\" (it is also the plugin's own systemMessage); got %q", terminal)
	}
}
