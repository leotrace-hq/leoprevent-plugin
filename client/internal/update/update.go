...
func HowLicense(
	...
	case "codex", "copilot":
		return "Ask your agent to run the installed LeoPrevent binary with login (remote sessions: login --no-browser). " +
			"Approve in your browser and wait for Connected to LeoPrevent."
	default:
		return "Connect with /leoprevent:login, approve in your browser, and wait for Connected to LeoPrevent. " +
...
//
// The two claude branches say different things because the developer's options really
// are different, and naming an action that cannot work is worse than naming none.
//
// DESKTOP has no agent-runnable path. The app keeps its marketplace in managed state
// that the standalone `claude` CLI cannot see, so a shelled-out `claude plugin update
// leoprevent@leotrace --scope managed` fails with "Marketplace leotrace not found" --
// and `claude plugin list` reports the install as "failed to load" for the same reason.
// A desktop install also carries autoUpdate, so by the time the nag is read the new
// build is usually already on disk and only a restart is missing. Hence: lead with the
// restart, and keep the click-path as the fallback for when that was not enough.
//
// TERMINAL does have one, so it leads with asking the agent rather than with the two
// slash commands the developer would otherwise type by hand. The slash commands stay,
// second, because the marketplace may have been added under a scope the agent cannot
// reach there either.
//
...
// restates as its own). "Ask me to update it" reads as nonsense in the first.
func How(
	agent string,
	environment string,
) string {
	...
	default: // claude
		if environment == wire.EnvClaudeDesktop {
			return "Update: LeoPrevent updates itself — restart Claude to pick it up. " +
				"If it is still behind after a restart, open Customize → Plugins → Browse → Code, " +
				"open the leotrace menu, then choose Check for updates."
		}
		return "Update: ask Claude to update the LeoPrevent plugin, or run " +
			"`/plugin marketplace update leotrace` yourself and update leoprevent in `/plugin`."
	}
}
