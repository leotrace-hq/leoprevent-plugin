...
func TestWordBoundaryPreventsSubstringFalsePositive(t *testing.T) {
	...
	if got["sensitive-data-in-logs"] {
		t.Error("sensitive-data-in-logs must not fire on fetch_url_content (substring false positive)")
	}
	if !got["ssrf"] {
...
func corpusIDs(t *testing.T) map[string]bool {
	...
	hardening, err := os.ReadFile(filepath.Join(dir, "hardening.yaml"))
	if err != nil {
		t.Fatalf("read hardening: %v", err)
	}
	rules, err := rulespec.ParseRules(patterns, hardening)
	if err != nil {
