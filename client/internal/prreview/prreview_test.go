...
// ⚠️ A CLEAN PULL REQUEST IS NOT COMMENTED ON, THE FIRST RUN INCLUDED. It carries no marker, so
// it read as a first run on every push and left one "No findings" comment per push. The step log
// is where a clean run is reported. See prpost.Assembled.Post for the reversal and its cost.
func TestACleanPullRequestIsNotCommentedOn(t *testing.T) {
	dir := branchWith(t, map[string]string{"app.py": "one\ntwo\nimport requests\n"})
	r := &fakeReviewer{}
	gh := &fakeGitHub{}
	...
	if len(gh.posted) != 0 {
		t.Fatalf("a clean run left a comment; got %+v", gh.posted)
	}
	if !strings.Contains(out.String(), "clean: no findings") {
		t.Errorf("the step log must still report the clean run; got:\n%s", out.String())
	}
}
