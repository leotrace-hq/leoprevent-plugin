...
func TestNoticeDeliveryBySurface(
	t *testing.T,
) {
	for _, entrypoint := range []string{"cli", "ssh-remote", "bench", "claude-desktop", "claude-desktop-3p", "remote_desktop", "remote", "remote_mobile", "claude-vscode", "local-agent", "sdk-cli", "unknown", "", "future-surface"} {
		t.Run(entrypoint, func(t *testing.T) {
			t.Setenv(entrypointEnv, entrypoint)
			a := New()
			env := a.Environment(agent.Event{}).Name
			findings := []wire.Finding{{Rule: "test"}}
			cases := []struct {
				name, message, context, event string
				render                        func() ([]byte, error)
			}{
				{"review", "verbatim review banner", review.ReviewContextMessage(1, findings, review.ForceFixedCount(findings)), "Stop", func() ([]byte, error) {
					return a.DeliverReview("authoritative findings", "verbatim review banner", 1, findings, "")
				}},
				{"prompt", "verbatim prompt notice", "relay prompt notice", "UserPromptSubmit", func() ([]byte, error) { return a.DeliverPromptNotice("verbatim prompt notice", "relay prompt notice") }},
				{"update", update.Message("claude", env, "0.2.11", "0.2.12"), update.ContextMessage("claude", env, "0.2.11", "0.2.12"), "UserPromptSubmit", nil},
				{"license", update.LicenseMessage("claude"), update.LicenseContextMessage("claude"), "UserPromptSubmit", nil},
			}
			for _, tc := range cases {
				t.Run(tc.name, func(t *testing.T) {
					...
					if tc.render != nil {
						...
						out, err = a.DeliverPromptNotice(tc.message, tc.context)
					}
					if err != nil {
						t.Fatal(err)
					}
					...
					if got["systemMessage"] != tc.message {
						t.Fatalf("verbatim notice changed: %s", out)
					}
					if tc.event == "Stop" {
						if got["decision"] != "block" || got["reason"] != "authoritative findings" {
							t.Fatalf("re-wake changed: %s", out)
						}
					} else if _, present := got["decision"]; present {
						t.Fatalf("prompt must not block: %s", out)
					}
					context, present := got["hookSpecificOutput"]
					terminal := entrypoint == "cli" || entrypoint == "ssh-remote" || entrypoint == "bench"
					if terminal {
						if present {
							t.Fatalf("terminal duplicates notice: %s", out)
						}
					} else {
						hso, ok := context.(map[string]any)
						if !ok || hso["hookEventName"] != tc.event || hso["additionalContext"] != tc.context {
							t.Fatalf("surface lost context: %s", out)
						}
					}
				})
			}
...
// The server's summary line is what the desktop notice states when it sent one, so the wording
// can change with a server deploy.
func TestDeliverReviewUsesTheServersSummary(t *testing.T) {
	t.Setenv(entrypointEnv, "claude-desktop")
	out, err := New().DeliverReview("FINDINGS", "banner", 1, []wire.Finding{{Rule: "sql-injection"}}, "Reviewed 1 changed file and said it the server's way.")
	if err != nil {
		t.Fatalf("DeliverReview: %v", err)
	}
	if !strings.Contains(string(out), "said it the server's way.") {
		t.Fatalf("server summary missing from the notice: %s", out)
	}
}
