...
func jsContext(root, relpath, src string, refs map[string]bool, workspaces *workspaceResolver) []candidate {
	dir := path.Dir(norm(relpath))
	var config *tsConfig
	var out []candidate
	add := func(clause, spec, mode string) {
		...
		if !isRelativeSpec(spec) {
			if config == nil {
				loaded := nearestTSConfig(root, dir)
				config = &loaded
			}
			candidates := config.candidates(root, spec)
		...
		out = append(out, labelled(clause != "", jsCandidates(root, dir, spec))...)
	}
...
func jsCandidates(root, dir, spec string) []string {
	p := join(dir, spec)
	out := []string{p}
	...
	if siblings, ok := jsSiblingExts[path.Ext(p)]; ok {
		if _, exists := safeRead(root, p); !exists {
