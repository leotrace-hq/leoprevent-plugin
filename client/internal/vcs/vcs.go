...
const excludeNodeModules = ":(top,exclude,glob)**/node_modules/**"
...
func CaptureBaseline(cwd, sessionID string) error {
	...
	sessionID = scopedSession(cwd, sessionID)
...
func snapshotUntracked(cwd string) string {
	...
	out, err := git(cwd, "ls-files", "--others", "--exclude-standard", "--", excludeNodeModules)
	if err != nil {
		return ""
	}
	var b strings.Builder
...
func ChangedFilesWithInfo(cwd, sessionID string) (changes []transcript.Change, ok bool, skip SkipReason, info BaselineInfo, err error) {
	...
	sessionID = baselineSession(cwd, sessionID)
	...
	if rerr != nil {
		if legacy := legacyBaseline(cwd, originalSession); legacy != nil {
			data, rerr = legacy, nil
		}
	}
...
type repoBaseline struct {
	...
	headOnly bool
	// rangeDiff marks a section comparing two COMMITS rather than a turn-start snapshot
	// against a working tree — the pull-request lane (see DiffRange). It skips the
	// untracked-file scan, because a pull request is committed code and anything else
	// lying in the checkout is the CI job's own doing: a build step that runs before the
	// review would otherwise have its output reviewed as though somebody had proposed it.
	//
	// It deliberately does NOT suppress the added-line numbers the way headOnly does.
	// Those numbers are real positions in the head commit and the lane needs them twice
	// over — the judge cites a line, and an inline review comment cannot be anchored
	// without one. What must not happen is those lines being read as authorship, and
	// that is the SERVER's job via wire.TurnMeta.ReviewLane, not this flag's: a guard
	// that lives only in the client binds a well-behaved client and nothing else.
	rangeDiff bool
}
...
func (cc *changeCollector) fromRepo(dir string, s repoBaseline) (SkipReason, int) {
	...
	if !s.rangeDiff {
		untracked, _ = git(dir, "ls-files", "--others", "--exclude-standard", "--", excludeNodeModules)
	}
...
func ClearBaseline(
	...
	_ = os.Remove(turnStartPath(sessionID))
...
func RecordEditedRepo(editPath, cwd, sessionID string) error {
	...
	if sessionID == originalSession {
		sessionID = scopedSession(cwd, originalSession)
	}
	switch {
	case !haveScratch:
...
// DiffRange collects the changes a pull request proposes: everything that differs
// between the MERGE BASE of base and HEAD, and HEAD itself. It is the pull-request
// lane's counterpart to ChangedFiles, and it is deliberately the same code — one
// repoBaseline handed to the same changeCollector — so the lane inherits every guard
// the hook path has rather than reproducing a subset of them: rename pairing, the
// symlink open-then-prove, the binary skip with its loud log line, the node_modules
// drop and the shared FullContent budget.
//
// ⚠️ THE ANCHOR IS THE MERGE BASE, NEVER THE BASE TIP. A two-dot diff against the tip
// of the target branch also reports everything that landed on that branch since the
// pull request forked — as though the PR were reverting it — so a busy repository
// would have its whole week's work reviewed on every PR, attributed to that PR, and
// commented on. `git merge-base` is what makes the diff the branch's own work.
//
// The base may be a branch name, a ref or a SHA; the caller passes whatever the CI
// event gave it. An empty base, a base the checkout cannot resolve (the commonest
// real cause being a shallow clone, which is the default for actions/checkout) or a
// directory that is not a repository all return a SkipReason and no changes — never a
// silent empty set, because on this lane an empty set is indistinguishable from a
// pull request that changed nothing reviewable.
func DiffRange(cwd, base string) (changes []transcript.Change, skip SkipReason, err error) {
	if cwd == "" || strings.TrimSpace(base) == "" {
		return nil, SkipNoCwdOrSession, nil
	}
	root := RepoRoot(cwd)
	if root == "" {
		return nil, SkipNotGitRepo, nil
	}
	// A shallow checkout is the default for actions/checkout, and it fails HERE with a
	// git error rather than by producing a wrong answer — which is why the skip reason
	// travels back to the caller for the copy to name fetch-depth.
	mb, mberr := git(root, "merge-base", strings.TrimSpace(base), "HEAD")
	if mberr != nil {
		return nil, SkipBaselineGone, nil
	}
	mb = strings.TrimSpace(mb)
	if mb == "" {
		return nil, SkipEmptyBaseline, nil
	}
	cc := &changeCollector{}
	if s, _ := cc.fromRepo(root, repoBaseline{ref: mb, rangeDiff: true, untracked: map[string]string{}}); s != "" {
		return nil, s, nil
	}
	return cc.changes, "", nil
}

// MergeBase resolves the commit a pull request forked from, for the copy that has to
// say what the review compared against. Empty when it cannot be resolved; callers
// treat that as "do not claim a base" rather than as an error, since DiffRange has
// already refused by then.
func MergeBase(cwd, base string) string {
	root := RepoRoot(cwd)
	if root == "" {
		return ""
	}
	mb, err := git(root, "merge-base", strings.TrimSpace(base), "HEAD")
	if err != nil {
		return ""
	}
...
// RevResolves reports whether a revision names something this checkout actually
// holds. The pull-request lane uses it to tell a bare base BRANCH name (which a
// pull_request checkout has only as a remote-tracking ref) from a SHA or a
// fully-qualified ref, which must be passed through untouched.
func RevResolves(cwd, rev string) bool {
	...
	_, err := git(root, "rev-parse", "--verify", "--quiet", rev+"^{commit}")
	return err == nil
}
