...
func TestSingleRepoBehaviourIsUnchanged(t *testing.T) {
	...
	raw, err := os.ReadFile(scratchPath(scopedSession(repo, s)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestClearBaselineRemovesDiscoveredRepos(t *testing.T) {
	...
	t.Cleanup(func() { ClearBaseline(base, s) })

	if err := RecordEditedRepo(filepath.Join(repo, "app.py"), base, s); err != nil {
		t.Fatal(err)
	}
	if entries, err := os.ReadDir(discoveredDir(scopedSession(base, s))); err != nil || len(entries) == 0 {
		t.Fatalf("nothing recorded to clear (err=%v)", err)
	}
	ClearBaseline(base, s)
	if _, err := os.Stat(discoveredDir(scopedSession(base, s))); !os.IsNotExist(err) {
