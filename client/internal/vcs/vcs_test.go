...
func initRepo(t *testing.T) (dir, session string) {
	...
	t.Cleanup(func() { _ = os.Remove(scratchPath(scopedSession(dir, session))) })
	return dir, session
}
...
func TestOldScratchWithoutAHistoryHeaderReviewsEverything(t *testing.T) {
	...
	data, rerr := os.ReadFile(scratchPath(scopedSession(dir, session)))
	if rerr != nil {
		t.Fatal(rerr)
	}
	first := strings.SplitN(string(data), "\n", 2)[0]
	if err := os.WriteFile(scratchPath(scopedSession(dir, session)), []byte(first+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
