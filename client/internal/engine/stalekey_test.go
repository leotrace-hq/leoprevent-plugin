...
func (noticeAgent) DeliverReview(string, string, int, []wire.Finding, string) ([]byte, error) {
	return nil, nil
}
...
type serverNoticeAgent struct{ noticeAgent }

func (serverNoticeAgent) DeliverNotice(
	message string,
) ([]byte, error) {
	return []byte(message), nil
}

func TestServerNoticeRenderingAndRecovery(
	t *testing.T,
) {
	isolateTempDir(t)
	withKey(t, "lp_live_servernotice")
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	event := agent.Event{SessionID: "server-notice-session"}
	var output bytes.Buffer
	notice := &wire.ServerNotice{ID: "arbitrary.future.reason", Message: "Server wording\nSecond line", ActionURL: "https://example.com/help"}
	skipped := &review.SkipError{Reason: review.SkipUnauthorized, Notice: notice, KeepKey: true}
	notifyReviewSkipped(serverNoticeAgent{}, event, skipped, log, &output)
	expected := notice.Message + " " + notice.ActionURL
	if output.String() != expected {
		t.Fatalf("message changed: %q", output.String())
	}
	if markerCount(t) != 0 {
		t.Fatal("server said key recovery will not help")
	}
	notifyReviewSkipped(serverNoticeAgent{}, event, skipped, log, &output)
	if output.String() != expected {
		t.Fatal("duplicate notice")
	}
	notice.ID = "another.reason"
	notifyReviewSkipped(serverNoticeAgent{}, event, skipped, log, &output)
	if output.String() != expected+expected {
		t.Fatal("distinct server notice was suppressed")
	}
	output.Reset()
	notifyReviewSkipped(serverNoticeAgent{}, agent.Event{SessionID: "legacy-fallback"}, &review.SkipError{Reason: review.SkipUnauthorized}, log, &output)
	if output.String() != review.SkipNotice(review.SkipUnauthorized) || markerCount(t) != 1 {
