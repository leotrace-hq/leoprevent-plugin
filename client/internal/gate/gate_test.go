...
func TestNotInert_UnknownAndBlockCommentTypes(t *testing.T) {
	...
	cases := []struct {
		...
		// NB style.css used to sit here as the unmapped-extension example. CSS is now
		// nonExecutable outright (see TestInert_Stylesheets), so it is no longer an
		// example of "unmapped, therefore reviewed". data.xyz still carries that case.
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if inert(transcript.Change{FilePath: tc.path, AddedText: tc.text}) {
...
func TestIsSecretPath(t *testing.T) {
	secret := []string{
		...
		// PREFIXED env files — the `<name>.env` shape (measured egressing in a customer
		// corpus). Same content class as .env, so the same never-egress rule.
		"prod.env", "staging.env", "dev/versions/2.4.8.env", `win\lane.env`,
	...
	notSecret := []string{
		...
		// `env` must be its own dot-separated tail, not a name that merely ends in
		// those three letters — otherwise real source stops being reviewed.
		"parseenv.go", "src/dotenv.ts", "loadenv.py", "env.go", "config/env.ts",
	}
...
func TestInert_MinifiedAndGeneratedSuffixes(t *testing.T) {
	gen := []string{
		...
		// `<name>.gen.<ext>` — hey-api/openapi-ts, oapi-codegen, sqlc, mockgen.
		"libs/api-client/src/client/types.gen.ts", "api/sdk.gen.ts",
		"internal/db/queries.gen.go", "mocks/store.gen.py", "lib/model.gen.dart",
	}
	for _, p := range gen {
	...
	// `.gen.` is matched only as the ANCHORED tail of the base name. A mid-name `.gen.`
	// or a `gen` path segment belongs to hand-authored code and must stay reviewable —
	// dropping it would blind the judge to real source, this gate's worst failure.
	authored := []string{
		"src/token.gen.helper.ts", "src/gen.service.ts", "gen/report_builder.go",
		"src/generator.ts", "codegen/config.ts", "src/regen.py",
	}
	for _, p := range authored {
		if inert(transcript.Change{FilePath: p, AddedText: "fetch(userControlledUrl)"}) {
			t.Errorf("inert(%q) = true, want false (hand-authored, only the .gen.<ext> tail is generated)", p)
		}
	}
	// Geographic/coordinate data dumps are not logic (sibling of .csv/.tsv).
	if !inert(transcript.Change{FilePath: "gis-zones/london_m25.geojson", AddedText: `{"type":"Polygon"}`}) {
		t.Error("inert(london_m25.geojson) = false, want true (coordinate data, not logic)")
	}
}
...
// TestInert_TestSupportFiles: test-tree SUPPORT files (page objects, helpers,
// fixtures, harness config) are dropped even though enry.IsTest — which matches on the
// FILE name — does not see them. Measured gap: a customer corpus egressed 41 such
// files, among them a totp.helper.ts and an auth.constants.ts of throwaway credentials.
func TestInert_TestSupportFiles(t *testing.T) {
	testOnly := []string{
		// Playwright / Cypress support trees, relative and (gitless fallback) absolute.
		"apps/frontend/admin-app/e2e/page-objects/login.page.ts",
		"apps/frontend/admin-app/e2e/helpers/totp.helper.ts",
		"apps/frontend/admin-app/e2e/helpers/auth.constants.ts",
		"apps/frontend/admin-app/e2e/fixtures/base-test.ts",
		"apps/frontend/admin-app/e2e/auth.setup.ts",
		"cypress/support/commands.js", "e2e/spec.ts",
		"/Users/dev/projects/app/apps/frontend/merchant-app/e2e/page-objects/create-order.page.ts",
		`win\proj\e2e\helpers\auth.helper.ts`,
		"libs/frontend/ui-web/src/lib/context-menu/__tests__/helpers.ts",
		// Harness config.
		"libs/backend/utils/tsconfig.spec.json", "dev/phpunit/phpunit.xml",
		"jest.setup.ts", "playwright.config.ts", "tests/conftest.py",
		// PHPUnit naming (enry covers *Test.java, not the PHP convention).
		"src/Test/Unit/Observer/Checkout/SubmitBeforeTest.php",
		"dev/fixtures/Tradeaze/DevFixtures/Test/Unit/Model/FixtureSpecTest.php",
		"src/Model/OrderTestCase.php",
	}
	for _, p := range testOnly {
		if !inert(transcript.Change{FilePath: p, AddedText: "await page.goto(url);"}) {
			t.Errorf("inert(%q) = false, want true (test-only support file)", p)
		}
	}
	// Guard rail — the segment list must not swallow first-party source. `test`/`tests`
	// is a real domain word, and a basename merely ENDING in the letters must not match.
	reviewable := []string{
		"src/tests/registry.ts",       // bare `tests` segment: NOT a drop trigger
		"data/dbt/tests/assert_x.sql", // dbt data tests live under a bare `tests`
		"src/latest.php", "src/Attest.php", "libs/e2eutils/src/index.ts",
		"src/app/order/order.router.ts", "src/protest/handler.go",
	}
	for _, p := range reviewable {
		if inert(transcript.Change{FilePath: p, AddedText: "db.query(userInput)"}) {
			t.Errorf("inert(%q) = true, want false (first-party source must stay reviewable)", p)
		}
	}
}

// TestInert_SVGActiveContent: an SVG is reviewable because it can carry a script, so
// the gate keeps every SVG that shows active content and drops only the decorative
// ones. This replaces the old premise that a big generated SVG is caught by the
// long-line heuristic — measured against customer corpora, that held for 2 of 10.
func TestInert_SVGActiveContent(t *testing.T) {
	// Active content in any form → reviewable. These are the SVG-XSS shapes.
	active := []string{
		`<svg><script>alert(1)</script></svg>`,
		`<svg onload="alert(1)"><rect/></svg>`,
		`<svg><rect onmouseover = "steal()"/></svg>`,
		`<svg><a href="javascript:alert(1)"><text>x</text></a></svg>`,
		`<svg><foreignObject><body xmlns="http://www.w3.org/1999/xhtml"><img src=x onerror=1></body></foreignObject></svg>`,
		`<svg><use href="https://evil.example/x.svg#a"/></svg>`,
		`<svg><image xlink:href="data:text/html;base64,PHN2Zz4="/></svg>`,
	...
	for _, body := range active {
		if inert(transcript.Change{FilePath: "assets/logo.svg", FullContent: body}) {
			t.Errorf("inert(svg) = true, want false (active content must stay reviewable): %.60s", body)
		}
	}
	// Decorative geometry, however large, carries nothing to review. leo-globe.svg is
	// 196 KB with a longest line of 107 chars, so the long-line heuristic never fires.
	big := strings.Repeat(`  <path d="M10 10 L20 20 Z" fill="#3b82f6"/>`+"\n", 4000)
	decorative := []string{
		`<svg viewBox="0 0 24 24"><path d="M4 4h16v16H4z"/></svg>`,
		`<svg xmlns="http://www.w3.org/2000/svg">` + big + `</svg>`,
		`<svg><g id="layer1"><circle cx="5" cy="5" r="4" stroke="#000"/></g></svg>`,
	}
	for _, body := range decorative {
		if !inert(transcript.Change{FilePath: "assets/logo.svg", FullContent: body}) {
			t.Errorf("inert(svg) = false, want true (decorative, no active content): %.60s", body)
		}
	}
	// A word merely CONTAINING "on" before an "=" is not an event handler.
	if inert(transcript.Change{FilePath: "a.svg", FullContent: `<svg version="1.1" pattern-notation="x"><rect/></svg>`}) != true {
		t.Error("inert(svg) = false, want true (version=/notation= are not on<name>= handlers)")
	}
	// An empty body proves nothing, so it must not be treated as inert.
	if svgWithoutActiveContent(transcript.Change{FilePath: "a.svg"}) {
		t.Error("svgWithoutActiveContent(empty) = true, want false (nothing to prove inertness with)")
	}
}

// TestInert_Stylesheets: stylesheets are presentation, and no corpus rule targets one,
// so sending them is pure cost (13.5% of one customer's egressed bytes). Component
// files that merely CONTAIN styles are not stylesheets and stay reviewable.
func TestInert_Stylesheets(t *testing.T) {
	for _, p := range []string{
		"leotrace-website/css/v4/home.css", "packages/design-system/src/components.css",
		"src/styles/planner.scss", "src/a.sass", "src/b.less", "src/c.styl", "src/d.pcss",
	} {
		if !inert(transcript.Change{FilePath: p, AddedText: ".a{color:red}"}) {
			t.Errorf("inert(%q) = false, want true (stylesheet)", p)
		}
	}
	// Guard rail: components carry logic alongside their styles.
	for _, p := range []string{
		"src/App.vue", "src/Card.svelte", "src/pages/index.astro",
		"src/theme.ts", "src/styles.js", "src/css.go",
	} {
		if inert(transcript.Change{FilePath: p, AddedText: "fetch(userControlledUrl)"}) {
			t.Errorf("inert(%q) = true, want false (component/source, not a stylesheet)", p)
		}
	}
}
