...
// CommitSHA's answer becomes the ref of every later call, so anything that is not a full hex SHA
// is refused rather than passed on.
func TestCommitSHAResolvesAndValidates(t *testing.T) {
	const sha = "0123456789abcdef0123456789abcdef01234567"
	c, got := capture(t, http.StatusOK, `{"sha":"`+sha+`"}`)
	s, err := c.CommitSHA("HEAD")
	if err != nil || s != sha {
		t.Fatalf("CommitSHA = %q, %v", s, err)
	}
	if *got != "/repos/acme/api/commits/HEAD" {
		t.Errorf("requested %s", *got)
	}
	for _, body := range []string{`{"sha":"HEAD"}`, `{"sha":"../../x"}`, `{}`, `not json`} {
		c, _ := capture(t, http.StatusOK, body)
		if s, err := c.CommitSHA("HEAD"); err == nil {
			t.Errorf("body %s: accepted %q", body, s)
		}
	}
}
