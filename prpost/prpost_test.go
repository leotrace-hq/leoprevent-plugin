...
func TestNoDashesInPostedCopy(t *testing.T) {
	...
	bodies := map[string]string{
		...
		"summary": SummaryBody(SummaryInput{Total: 1, Inline: 1, AlreadyPosted: 1, FilesReviewed: 2, Unanchored: []wire.Finding{f}, Base: "main"}),
...
// TestACleanReviewStillSaysItCoveredPartOfTheChange is the carve-out, and it is the half of the
// change that must not be lost to a later tidy-up of the one above.
//
// Dropping the footers is about findings the review does not have. Covering only PART of the
// pull request is a different claim entirely: a partial review presented as a whole one is the
// failure every bounded read in this codebase is written to prevent, and it is strictly MORE
// important on a clean pass, because "no findings" is what a reader would otherwise take it to
// qualify.
func TestACleanReviewStillSaysItCoveredPartOfTheChange(t *testing.T) {
	...
	if !strings.Contains(body, "larger than this review covers") {
		t.Fatalf("a clean review dropped the truncation notice along with its footers:\n%s", body)
	}
}

// TestTheSummaryNeverListsTheExcludedFiles pins a REMOVAL, because restoring it reads as an
// improvement.
//
// The summary used to name every path the inert gate dropped: documentation, lockfiles,
// generated output, vendored trees and, on a normal pull request, most of its test files. That
// is a list nobody reads, sitting above the findings a reviewer opened the comment for, and on a
// change touching a few dozen files it was several screens of it. The information is not lost:
// both drivers log the excluded paths (prreview to the step log, prlane to the server log), which
// is where a wrongly excluded file gets noticed. Only the posted comment changed.
func TestTheSummaryNeverListsTheExcludedFiles(t *testing.T) {
	f := wire.Finding{Rule: "ssrf", Name: "SSRF", Severity: "high", Location: "app.py:3", Issue: "i", Fix: "f"}
	bodies := map[string]string{
		"with findings": SummaryBody(SummaryInput{Total: 1, Inline: 1, FilesReviewed: 2, Unanchored: []wire.Finding{f}, Base: "main"}),
		"clean":         SummaryBody(SummaryInput{FilesReviewed: 9, Base: "main"}),
		"truncated":     SummaryBody(SummaryInput{FilesReviewed: 9, Base: "main", Truncated: true}),
	}
	for name, body := range bodies {
		for _, banned := range []string{"Not reviewed", "inert gate", "lockfiles", "vendored"} {
			if strings.Contains(body, banned) {
				t.Errorf("the %s summary lists what was excluded again (%q):\n%s", name, banned, body)
			}
		}
	}
}
...
// TestNothingReviewedDoesNotReadAsAClean stops a pull request nothing looked at from borrowing
// the wording of one that came back clean.
//
// "No findings on the 0 reviewed files" is literally true and reads as a clean bill of health.
// It used to be followed by the list of excluded paths, which explained it; with that list gone
// the sentence has to explain itself.
func TestNothingReviewedDoesNotReadAsAClean(t *testing.T) {
	body := SummaryBody(SummaryInput{FilesReviewed: 0, Base: "main"})
	if strings.Contains(body, "0 reviewed files") {
		t.Errorf("a pull request nothing reviewed reports itself as clean:\n%s", body)
	}
	if !strings.Contains(body, "needed a security review") {
		t.Errorf("a pull request nothing reviewed does not say so:\n%s", body)
	}
}
