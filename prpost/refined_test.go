...
// ⚠️ THE FIRST RUN ALWAYS POSTS, INCLUDING A CLEAN ONE. New is zero on a clean pull request, and
// a clean review is the one result a reader most needs stated — silence is indistinguishable
// from a lane nobody configured.
func TestACleanFirstRunStillPosts(t *testing.T) {
	a := Assemble(AssembleInput{FilesReviewed: 1})
	if a.New != 0 {
		t.Fatalf("New = %d, want 0 on a clean run", a.New)
	}
	if !a.Post(nil) {
		t.Error("a clean first run said nothing at all")
	}
}
