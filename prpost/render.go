...
func SummaryBody(in SummaryInput) string {
	...
	switch {
	// NOTHING REVIEWED IS NOT A CLEAN RESULT, AND IT MUST NOT BORROW ONE'S WORDING.
	// "No findings on the 0 reviewed files" is literally true and reads as a clean bill of
	// health on a pull request nothing looked at. It used to be followed by the list of
	// excluded paths, which explained it; with that gone the sentence has to explain itself.
	case in.FilesReviewed == 0:
		b.WriteString("Nothing in this pull request needed a security review. Its changed files are documentation, lockfiles, generated output, tests or vendored code.\n\n")
	case in.Total == 0:
	...
	//
	// ⚠️ THE SECTIONS ABOVE ARE NOT FOOTERS AND ARE DELIBERATELY OUTSIDE THIS: a clean review
	// that was TRUNCATED still says it covered part of the change. That states what was not
	// looked at, which is exactly the claim a bare "no findings" would otherwise overstate.
	if in.Total == 0 {
		return b.String()
	}
...
type Assembled struct {
	...
	// first time. It is the WHOLE of what `Post` reads; see there for why a run with none
	// posts nothing, a clean one included.
	New int
...
//
// ⚠️ A RUN WITH NOTHING NEW POSTS NOTHING AT ALL, AND A CLEAN REVIEW IS A RUN WITH NOTHING NEW.
// The marker de-dup already stops an inline comment repeating, but the summary body was posted
...
//
// ⚠️ THIS REVERSES "THE FIRST RUN ALWAYS POSTS, INCLUDING A CLEAN ONE", AND THE OLD RULE'S
// REASONING IS WORTH KNOWING BEFORE ANYBODY RESTORES IT. That rule held that a clean review is
// the one result a reader most needs stated, because silence is indistinguishable from a lane
// nobody configured. What it missed is that a clean review posts NO marker — the markers ride
// the inline comments and the unanchored findings — so `PostedMarkers` reads back an empty map
// on the next push and `len(posted) == 0` was true EVERY TIME: a pull request that stays clean
// collected one "No findings" comment per push, which is the repetition the paragraph above
// exists to prevent. Reported by a customer (Tradeaze, 2026-09-16) against two identical clean
// comments seven minutes apart, and the answer they asked for is a CHECK RUN rather than a
// comment, so the comment goes and the check is the follow-up.
//
// ACCEPTED COSTS, stated rather than hidden. (1) Until the lane reports a check run, a customer
// cannot tell a clean review from a lane that never ran; that is the trade the reversal makes
// and the reason the check is the follow-up. (2) A re-run that changes only the truncation
// notice says nothing, because no finding is new — so a pull request that is clean AND
// truncated never states that some of its added lines went unread. Fixing that needs the notice
// to carry a marker of its own, which is what would let it be said exactly once.
func (a Assembled) Post() bool {
	return a.New > 0
}
