...
func TestClientLogIsBoundedByRotation(t *testing.T) {
	path := filepath.Join(t.TempDir(), "client.log")
	t.Setenv("LEOPREVENT_LOG", path)
	if err := os.WriteFile(path, make([]byte, maxClientLogBytes), 0o600); err != nil {
		t.Fatal(err)
	}
	w, closeFn := openWriter("client")
	_, _ = fmt.Fprintln(w, "new generation")
	closeFn()
	if _, err := os.Stat(path + ".1"); err != nil {
		t.Fatalf("backup: %v", err)
	}
	if info, err := os.Stat(path); err != nil || info.Size() >= maxClientLogBytes {
		t.Fatalf("new log was not bounded: %v %v", info, err)
	}
}
