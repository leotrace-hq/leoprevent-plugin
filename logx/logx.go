...
// Client logs may contain the developer's own code when audit bodies are enabled. Keep one
// bounded generation instead of allowing that local copy to grow for the life of a machine.
const maxClientLogBytes int64 = 5 << 20
...
func rotateClientLog(path string) {
	info, err := os.Stat(path)
	if err != nil || info.Size() < maxClientLogBytes {
		return
	}
	backup := path + ".1"
	_ = os.Remove(backup)
