...
type TurnMeta struct {
	...
	// ReviewLane names WHERE this review ran when it was NOT a developer's own turn.
	// Absent — every ordinary Stop-hook turn — means the diff IS a turn's work. The one
	// value today is LanePullRequest: the CI lane that reviews a pull request's diff
	// (LEO-231).
	//
	// ⚠️ IT IS THE SERVER'S AUTHORITY THAT NOTHING HERE WAS "INTRODUCED", AND THAT IS
	// WHY IT IS ITS OWN FIELD RATHER THAN A READING OF `Environment`. A pull request's
	// added lines are the BRANCH's — possibly several people's, over days — so the
	// positional anchor that decides introduced-vs-pre-existing is answering a question
	// this diff cannot support. api.surfaceAll therefore marks every finding on a laned
	// review PRE-EXISTING, i.e. surfaced, and api.preexistingInstruction sends no
	// remediation directive. Keying that on an environment string would mean a second CI
	// runtime added later silently got introduced classification — a safety regression
	// wearing a new value's clothing — whereas a lane field a new driver has to set is a
	// guarantee by construction.
	//
	// Metadata, always retained. It also QUALIFIES EVERY FINDING COUNT on the event, the
	// same way HeadAnchoredRepos does: a laned review is not comparable to a turn's, so
	// packages/metrics keeps it out of every per-developer and per-repo figure and both
	// consoles badge the record.
	ReviewLane string `json:"review_lane,omitempty"`
	// PullRequestNumber and PullRequestURL identify the pull request a LanePullRequest
	// review ran over, so a finding in the log is traceable back to the review it was
	// posted on. Both are public repository metadata, never a path and never PII; the
	// URL is the same one the badge links to. Absent on every other lane.
...
type Finding struct {
	...
	// AikidoIssueID / AikidoURL / AikidoRole link a finding to an issue in the customer's
	// Aikido workspace (LEO-332). SERVER-ONLY, like SecretValues: never serialised, so the
	// client neither receives nor echoes them. The server recognises an Aikido-sourced finding
	// on /outcome by its rule namespace instead (`aikido:` / `aikido-fp:`).
	AikidoIssueID int64  `json:"-"`
	SnykIssueID   string `json:"-"`
	AikidoURL     string `json:"-"`
	SnykURL       string `json:"-"`
	AikidoRole    string `json:"-"`
	SnykRole      string `json:"-"`
...
// per input. Resist it: every value here becomes a row in someone's UI, and a surface
// nobody actually runs would be a row that is always zero. A recognized-but-unmapped
// input lands on EnvUnknown with TurnMeta.EnvironmentRaw carrying what it actually
// was, which is the honest record and costs no UI. Add a constant when the log shows
// the raw value arriving, not in anticipation of it.
//
// ⚠️ EnvGitHubActions IS THE ONE THAT WAS ADDED BY BEING EARNED, and the earlier
// version of this note named it as the example of a row that would always be zero —
// on the reasoning that nobody installs a Stop hook in an ephemeral CI sandbox. That
// reasoning was right and is now superseded by LEO-231: the pull-request lane runs
// our own binary there deliberately, with no hook and no agent, so the runtime is a
// real surface with real turns in it. Read it as the rule working rather than as the
// rule being bent — the constant landed when the value started arriving.
const (
	...
	// EnvGitHubActions is a GitHub Actions runner — no agent, no developer at a
	// keyboard, no Stop hook. Today that is only the pull-request lane, which also sets
	// ReviewLane; the two are separate axes on purpose, because the RUNTIME (this) and
	// the REASON the review ran (the lane) answer different questions, exactly as Agent
	// and Environment do. A second CI provider gets its own constant when its raw value
	// shows up; until then it reads EnvUnknown with EnvironmentRaw naming it.
...
// TurnMeta.ReviewLane values — the closed vocabulary for a review that was NOT a
// developer's own turn. Absent is the ordinary Stop-hook case and is deliberately not
// a constant: "no lane" is the overwhelming majority and must cost no bytes on the
// wire, so there is nothing to spell.
//
// ⚠️ THE SERVER'S SURFACED-ONLY GUARANTEE KEYS ON "ANY LANE", NEVER ON A SPECIFIC
// VALUE. A driver that reviews something other than one developer's turn cannot
// support the positional introduced-vs-pre-existing anchor, whatever it is reviewing —
// so a new value added here inherits the guarantee rather than needing to be added to
// a list somewhere. Read api.surfaceAll's own comment before narrowing that.
const (
	// LanePullRequest is the CI lane that reviews a pull request's diff and posts its
	// findings as review comments (LEO-231). Advisory: it never blocks, never re-wakes
	// an agent, and never ships an outcome — there is no agent to fix anything and no
	// second Stop to score.
	LanePullRequest = "pull_request"
)
...
type ReviewResponse struct {
	...
	// Summary is the SERVER-AUTHORED one-line account of the review that the client shows the
	// developer at the top of the agent's reply ("Reviewed 2 changed files and raised 1
	// finding, plus 1 Aikido finding to double-check. See below."). Empty means the client uses
	// its own compiled-in wording, which is what every client did before this field existed.
	//
	// ⚠️ ON THE WIRE FOR PreexistingDirective's REASON: the sentence changes whenever the review
	// learns to report something new (Aikido disputes were the first), and compiled into the
	// client each change needed a plugin release plus every install updating. It describes ONE
	// request, so a client that split the turn across several /review batches ignores it and
	// composes its own. The client renders it verbatim after sanitising it to one bounded line;
	// the byline and the instruction around it stay client-side, because those are how the
	// notice is delivered rather than what it says.
	Summary string `json:"summary,omitempty"`
	// ContextHint is a SERVER-AUTHORED sentence the client appends, verbatim and last, to a
	// BLOCKED turn's re-wake: a request that the agent mention, at a convenient moment, that
	// connecting GitHub would give reviews of this repository more context (LEO-161). The server
	// decides whether to send it (the repository has no profile and the account has not
	// connected GitHub); the client only throttles it to once per session. Empty ⇒ nothing is
	// appended, which is what every client did before this field existed.
	//
	// ⚠️ IT NEVER CAUSES A BLOCK. It rides a re-wake a security finding already earned, because
	// that is the only channel to the agent; a clean turn carries none.
...
// AikidoDisputeRulePrefix namespaces a finding the review put to the agent as a LIKELY FALSE
// POSITIVE in the customer's Aikido workspace (LEO-332). It is suggest-only and counted nowhere,
// so the client names it apart from the findings it raises, and the server recognises one on
// /outcome by this prefix, since the Aikido link fields never cross the wire. One definition, so
// the two sides cannot disagree about which findings are disputes.
const AikidoDisputeRulePrefix = "aikido-fp:"

const SnykDisputeRulePrefix = "snyk-fp:"
