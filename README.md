...
```
Recent versions report **Plugin is now active** and need nothing further. Only if
`/leoprevent:login` reads as an unknown command, reload (or just start a new session):
```
/reload-plugins
```
...
Your coding assistant needs its own working sign-in before any of this: connecting to
LeoPrevent does not sign you in to Claude, and an assistant that cannot run cannot set this up
for you.
...
## Reinstalling, or setting up a second machine

Most of the first-time steps are not repeated:

- **The marketplace stays registered.** Adding it again is harmless and reports that it is already
  there.
- **Your connection survives a same-version reinstall.** The credential lives in your user config
  dir, outside the plugin, so reinstalling does not disconnect you. A reinstalled plugin has been
  verified running a real review on the saved connection with no second login.
- **Codex hook trust survives a same-version reinstall**, but not a version whose hooks changed.
  Codex asks again then, and performs no reviews until you answer. Review it rather than assuming.
- **A new machine is a new connection.** Run `login` there; it does not take a key away from any
  machine you already use.
- **A cloud session is the exception.** Its credential lives on the remote filesystem, so run
  `login` again whenever that environment is recreated.

However you got here, the check is the same: work in a git repo, finish a turn, and confirm the
review ran. An uninvoked hook looks exactly like a quiet turn, so a hook you ran by hand is not
evidence that your client is firing it.
...
Remote/headless and other platforms use the text confirmation.

**Cloud sessions are the exception: use a license key, not browser login.** The credential login
saves lives on the remote filesystem, which is rebuilt, so login is a step you would have to
notice and repeat every time, with an unconnected hook failing open in between. Set
`LEOPREVENT_LICENSE_KEY` as an environment variable instead, as under
[On the web](#on-the-web-claudeaicode); the rebuild carries it. (If you do run login there, allow
`prevent.leotrace.io` as well as `api.leotrace.io`: that is where the device flow starts and
polls. Reviews themselves only ever talk to `api.leotrace.io`.)
