...
const (
	...
	// Client (review) — bound the server's repository-context reminder
	// (wire.ReviewResponse.ContextHint) appended to a re-wake. It is one server-authored
	// sentence, so this is a ceiling on a misbehaving server, not a budget for the copy:
	// the SERVER's own test asserts its sentence fits, because a sentence cut to this
	// bound ends mid-word in front of the agent and nothing else would say so.
	MaxContextHintRunes = 600
