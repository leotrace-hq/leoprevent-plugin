...
func TestTypeScriptPathAliases(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := paths(resolve(t, root, Change{FilePath: tt.file, AddedText: "verifyAdminToken()", FullContent: "import { verifyAdminToken } from '" + tt.spec + "';"}))
...
func TestTypeScriptAliasTargetsAndGates(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := paths(resolve(t, root, Change{FilePath: "app.ts", FullContent: tt.source, AddedText: tt.added}))
			if !reflect.DeepEqual(got, tt.want) {
				t.Fatalf("got %v, want %v", got, tt.want)
			}
		})
...
func TestTypeScriptConfigInheritance(
	...
	for _, configs := range []map[string]string{
		...
		got := paths(resolve(t, root, Change{FilePath: "app.ts", FullContent: "import { verifyAdminToken } from '@/auth';", AddedText: "verifyAdminToken()"}))
...
func TestTypeScriptAliasSymlinkEscape(
	...
	for _, configLink := range []bool{false, true} {
		...
		got := resolve(t, root, Change{FilePath: "app.ts", FullContent: "import { verifyAdminToken } from '@/linked/auth';", AddedText: "verifyAdminToken()"})
		if len(got) != 0 {
...
func TestTypeScriptNearestUnsafeConfigDoesNotUseParent(
	...
	got := resolve(t, root, Change{FilePath: "src/app.ts", FullContent: "import { verifyAdminToken } from '@/auth';", AddedText: "verifyAdminToken()"})
	if len(got) != 0 {
