...
func nearestTSConfig(
	source Source, dir string,
) tsConfig {
	for inRepoConfigPath(dir) {
		rel := join(dir, "tsconfig.json")
		if source.Exists(rel) {
			budget := 64
			config, _ := loadTSConfig(source, rel, map[string]bool{}, &budget)
			return config
		}
		if dir == "." {
			break
...
func loadTSConfig(
	...
	*budget -= 1
	...
	for _, base := range bases {
		basePath := tsConfigBase(source, path.Dir(rel), base)
		parent, valid := loadTSConfig(source, basePath, visiting, budget)
		if !valid {
			return tsConfig{}, false
		}
...
func tsConfigBase(
	...
	for inRepoConfigPath(dir) {
		...
		for _, candidate := range []string{rel, rel + ".json"} {
			if _, ok := source.Read(candidate); ok {
				return candidate
			}
		}
		if body, ok := source.Read(join(rel, "package.json")); ok {
			var pkg struct {
		...
		if _, ok := source.Read(join(rel, "tsconfig.json")); ok {
...
func (config tsConfig) candidates(
	...
	for _, target := range config.paths[best] {
		...
		for _, rel := range jsCandidates(source, base, target) {
