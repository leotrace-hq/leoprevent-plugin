...
//   - Direct dependencies only; JS workspace exports may follow bounded relative
//     re-export chains to a definition, but never arbitrary runtime imports.
...
//   - Read-only, guarded — secret files (pathgate.IsSecretPath) and symlinks are
...
func Resolve(root string, changes []Change) []wire.ContextFile {
	if root == "" || len(changes) == 0 {
		return nil
	}
	return ResolveFrom(osSource{root: root}, changes)
}

// ResolveFrom is Resolve over any Source — the checkout-free entry point.
//
// Resolve delegates to it with an osSource, so a checkout takes exactly the path it always
// did and every existing test of this package exercises the real behaviour: if they pass,
// the local path did not change.
func ResolveFrom(source Source, changes []Change) []wire.ContextFile {
	if source == nil || len(changes) == 0 {
		return nil
	}
	...
	// npm workspace links live in node_modules, which is never committed — so a
	// tree-backed Source has none to find and the resolver is simply not built. JS
	// RELATIVE imports still resolve, through Source.Read probing, which is the common case.
	var workspaces *workspaceResolver
	if source.Local() {
		workspaces = &workspaceResolver{root: source.(osSource).root}
	}
	var idx *repoIndex // built lazily, only if an index-based language appears
	getIdx := func() *repoIndex {
		if idx == nil {
			idx = buildIndex(source)
		}
		return idx
	}
	...
	for _, c := range changes {
		...
		case langPython:
			cands = pythonContext(source, c.FilePath, src, refs)
		case langJS:
			cands = jsContext(source, c.FilePath, src, refs, workspaces)
		case langJava:
...
// (pathgate.IsSecretPath). ok=false on any failure → the file is simply skipped.
